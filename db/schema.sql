...
CREATE TABLE IF NOT EXISTS matches (
    match_id        INTEGER PRIMARY KEY,      -- id de la API
...
CREATE TABLE IF NOT EXISTS match_player_stats (
    ...
    team_side       TEXT,                     -- allied | axis | unknown
    ...
    -- JSONB para armas, víctimas frecuentes, etc.
...
SELECT
    ...
    ROUND(
        SUM(mps.kills)::NUMERIC /
        NULLIF(SUM(mps.deaths), 0), 2
    )                                                   AS overall_kd,
...
GROUP BY p.player_id, p.name, p.country, p.last_seen_at;
